        ...
        new Thread(() -> {
            while (true) {
                try {
                    ...
                    ProcessRequisition processRequisition = getProcesses().get((int) (Math.random() * getProcesses().size())).createRequisition();
                    getProcesses().forEach(p -> p.receiveRequisition(processRequisition));
                    getProcesses().stream().filter(p -> p.getId() == processRequisition.getId()).findAny().ifPresent(Process::setCoordenator);
